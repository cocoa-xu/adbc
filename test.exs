...
{:ok, schema} = Adbc.Nif.export_float32_utf8_type()
{:ok, array} = Adbc.Nif.export_float32_utf8_array()
schema = %Adbc.ArrowSchema{reference: schema}
array = %Adbc.ArrowArray{reference: array}
:ok = Statement.bind(statement, array, schema)
{:ok, _stream, _row_affected} = Statement.execute_query(statement)
